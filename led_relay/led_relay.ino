...
Hardware connections:
  ...
  Diode:
    ...
    Connect the other side of the diode (anode) to the COLLECTOR pin of the transistor.
...
boolean ledState=false;         // the present state of the LEDs (ON/OFF)
...
// Checking if the button was pressed and alternating LEDs
void checkButton()
{
  currentButton = debounce(testButton,lastButton);
  if (currentButton == HIGH && lastButton == LOW) // button was pressed
     ledState =! ledState; //toggle the LEDs state
   lastButton = currentButton;
   digitalWrite(relayPin, ledState); // Alternate relay state
}